...
public class SpanTest {
    ...
    public void testRootSpan() {
        ...
        final Span span = Span.start(new SpanInfo("testSpan"));
        ...
        assertEquals("testSpan", span.getInfo().getName());
        assertTrue(span.getDuration() >= 0);
    }

    @Test
    public void testRootlessSpans() {
        final Span outer = Span.start(new SpanInfo("outerSpan"));
        final Span inner = Span.start(new SpanInfo("innerSpan"));
        inner.end();
        outer.end();
    }
    ...
    private class BarrierSpanRunner implements Runnable {
        ...
        public void run() {
            try {
                final Span span = Span.start(new SpanInfo(spanName));
                barrier.await();
                span.end();
            } catch (Exception e) {
